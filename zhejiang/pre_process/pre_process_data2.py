...
Polarities_dic = {'正面': 'PP', '负面': 'PN','中性':'PU'}
...
def count_category(output_file):
    ...
    for v, k in enumerate(set(cates)):
        cates_ids[k] = v
    logger.info(cates)
    pd.Series(cates_ids).to_csv("./category_ids.csv")
...
def data_for_squence2(input_file, output_file=None):
    ...
    if not output_file:
        ...
        f = lambda list_words: "\n".join(list_words)
        sentences = list(map(f, sentences))
        with open(r"./preprocess_result_data/test.txt", mode="w", encoding="utf-8") as file:
            file.write("\n\n".join(sentences))
            file.close()
    else:
        df_labels = pd.read_csv(output_file, encoding="utf-8", delimiter=",", header=0)
        ...
        text = ""
        cols_name = "AspectTerms,A_start,OpinionTerms,O_start,Categories,Polarities".split(",")
        for col_id, col_review in tqdm(df_reviews[["id", "Reviews"]].values):
            # logger.info(col_id)
            # logger.info(col_review)
            col_id_df = df_labels.loc[df_labels.id == col_id]
            ...
            # print(cols_name)
            for AspectTerms, A_start, OpinionTerms, O_start, Categories,Polarities in col_id_df[cols_name].values:
                ...
                if OpinionTerms != "_":
                    sentiment = Polarities_dic[Polarities]
                    obf = "m_"+sentiment  # 修饰自身
                    try:
                        A_start = int(A_start)
                        O_start = int(O_start)
                        if A_start < O_start:
                            obf = "f_"+sentiment  # 修饰前面aspect
                        else:
                            obf = "b_"+sentiment  # 修饰后面aspect
                    except:
                        pass
        ...
        with open(r"./preprocess_result_data/dev.txt", mode="w", encoding="utf-8") as file:
            file.write("\n\n".join(text_dev))
            file.close()

        with open(r"./preprocess_result_data/train.txt", mode="w", encoding="utf-8") as file:
            file.write("\n\n".join(text_train))
            file.close()
...
if __name__ == '__main__':
    file_labels = r"/Users/mohaoran/Documents/Competition/zhijiang_sentiments/bert/zhejiang/pre_process/TRAIN/Train_labels.csv"
    file_reviews = r"/Users/mohaoran/Documents/Competition/zhijiang_sentiments/bert/zhejiang/pre_process/TRAIN/Train_reviews.csv"
    file_reviews_ = r"/Users/mohaoran/Documents/Competition/zhijiang_sentiments/bert/zhejiang/pre_process/TEST/Test_reviews.csv"
    ...
    # for test
    data_for_squence2(file_reviews_)
    # for train
